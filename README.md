...
### โครงสร้างของฟังก์ชัน `calculateSolutions`:
- **การเริ่มต้น:** ตัวเลขที่ได้รับจะถูกแปลงเป็นคู่ (pair) ของตัวเลขและการแสดงผล (expression)
- **การค้นหาคำตอบ:** ฟังก์ชัน `findSolutionsRecursive` จะถูกเรียกใช้เพื่อทำการค้นหาคำตอบที่เป็นไปได้จากตัวเลขที่เหลือ
- **การคำนวณ:** ฟังก์ชันจะพิจารณาทุกความเป็นไปได้ของการใช้ **operator** ระหว่างตัวเลขทุกคู่และหาคำตอบที่เป็นไปได้
- **ผลลัพธ์:** คำตอบทั้งหมดจะถูกเก็บไว้ใน **Set** เพื่อป้องกันการซ้ำซ้อน และหากพบคำตอบที่เท่ากับ 24 จะส่งคืนคำตอบนั้น
...
1. **`POST /api/signup`**  
   ...
   **ข้อมูลที่ต้องการ:** `username`, `password`  
   **การเก็บข้อมูล:** ทำการ Hash password ก่อนบันทึกลงฐานข้อมูล

2. **`POST /api/signin`**  
   สำหรับเข้าสู่ระบบด้วย `username` และ `password`

3. **`POST /api/signout`**  
   สำหรับออกจากระบบ

4. **`GET /api/get-answers`**  
